...
public class UtilisateurManager {
	...
	private void checkFields(Utilisateur utilisateur) throws BLLException {
		...
		if (utilisateur.getNom().isBlank())
			throw new BLLException("Le champs nom est obligatoire !");
		if (utilisateur.getPrenom().isBlank())
			throw new BLLException("Le champs prénom est obligatoire !");
		if (utilisateur.getEmail().isBlank())
			throw new BLLException("Le champs email est obligatoire !");
		if (utilisateur.getTelephone().isBlank())
			throw new BLLException("Le champs téléphone est obligatoire !");
		if (utilisateur.getRue().isBlank())
			throw new BLLException("Le champs rue est obligatoire !");
		if (utilisateur.getCodePostal().isBlank())
			throw new BLLException("Le champs code postal est obligatoire !");
		if (utilisateur.getVille().isBlank())
			throw new BLLException("Le champs ville est obligatoire !");

		if (!StringUtils.isAlphanumeric(utilisateur.getPseudo()))
			throw new BLLException("Pas de charactères spéciaux merci !");
